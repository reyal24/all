...
async def main():
	...
	#DOGE_click_bot_JOIN
	print(f'NAMA AKUN : {me.first_name}({me.username})')
	print(f'NOMOR HP : ({phone_number})\n')
	
	print(Fore.YELLOW + ' #DOGECOIN  #LITECOIN  #BITCOIN CASH  #ZCASH COIN' + Fore.RESET)
	# Start command /join
	...
	async def join_start(event):
		...
		if 'You must join' in message:	
			...
			print_msg_time(Fore.BLUE + f'BERGABUNG KE @{channel_name}...' + Fore.RESET)
	...
	async def wait_hours(event):
		...
		if 'You must stay' in message:	
			...
			print_msg_time(Fore.GREEN + f'SUCCES !!! waiting {waiting_hours} for rewards' + Fore.RESET)
			time.sleep(2)
			# No more ads
	@client.on(events.NewMessage(chats=dogeclick_channel, incoming=True))
	...
	async def join_start(event):
		...
		if 'Forward a message to' in message:	
			...
			print_msg_time(Fore.BLUE + f'URL @{channel_msg}' + Fore.RESET)
			...
			print_msg_time(Fore.BLUE + f'MESSAGING @{channel_name}...' + Fore.RESET)
	...
	async def earned_amount(event):
		...
		if 'You earned' in message:	
			print_msg_time(Fore.GREEN + event.raw_text+ Fore.RESET)
			time.sleep(2)
			# No more ads
	@client.on(events.NewMessage(chats=dogeclick_channel, incoming=True))
	async def no_ads(event):
	...
	async def join_start(event):
		...
		if 'Forward a message to' in message:	
			channel_msg = event.original_update.message.reply_markup.rows[0].buttons[0].url
			print_msg_time(Fore.GREEN + f'URL @{channel_msg}' + Fore.RESET)
